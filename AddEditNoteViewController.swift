...
class AddEditNoteViewController: UIViewController {
    ...
    // 32. Add madatory membors to hold called view object and note value
    ...
    // 19. Function to Dismiss current view when cancel button pressed
    func cancelButtonPressed(){
        self.dismissViewControllerAnimated(true, completion: nil)
    }
    
    
    // 23. Add or Update note when save button pressed.
    func saveButtonPressed(){
        ...
        if myTextField.text == "" {
            ...
            if (self.note != nil) {
                ...
                self.note?.contents = myTextArea.text
                self.note?.priority = Int(myStepper.value)
                self.note?.createdAt = NSDate()
                ...
                // ADD MODE.
                let note = Note(id: ++count, title: myTextField.text!, contents: myTextArea.text, priority: Int(myStepper.value), created_at: NSDate())
    ...
    // 17. Add Custom view to set UI objects positions and values if edit mode.
    func addCustomView(note: Note?) {
        
        // 18. Add Nav bar Cancel button.
        ...
        // 20. Set UI object values
        if let noteData = note {
            myTextField.text = noteData.title
            if let contents = noteData.contents{
                myTextArea.text = contents
            }
            myStepper.value = Double(noteData.priority)
        }
        
        // 21. Set posions to each UI object by NSLayoutConstraints.
